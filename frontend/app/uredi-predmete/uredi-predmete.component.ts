...
@Component({
  selector: 'app-uredi-predmete',
  templateUrl: './uredi-predmete.component.html',
  styleUrls: ['./uredi-predmete.component.css']
})
export class UrediPredmeteComponent implements OnInit {
  ...
  ukloni(predmet:Predmet){
    for(let i=0;i<this.sviPredmeti.length;i++){
      if(predmet.naziv==this.sviPredmeti[i].naziv){
        this.sviPredmeti.splice(i,1);
      }
    }
    for(let i=0;i<this.mojiPredmeti.length;i++){
      if(predmet.naziv==this.mojiPredmeti[i].naziv){
        this.mojiPredmeti.splice(i,1);
      }
    }
   
    this.userService.removePredmet(predmet.naziv).subscribe((data:Predmet)=>{
      // alert(1);
     });
    alert(7);
  }
